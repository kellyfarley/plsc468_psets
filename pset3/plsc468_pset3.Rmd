...
```{r}
temp_lda = textProcessor(documents = ads$texts, metadata = ads[, 2:7]) # clean data
outs_lda = prepDocuments(temp_lda$documents, temp_lda$vocab, temp_lda$meta) # eliminates very common and very rare terms
```
...
findingk <- searchK(outs_lda$documents, outs_lda$vocab, K = c(5, 10, 15, 20, 50),
 prevalence =~ party + tone, data = outs_lda$meta, verbose=FALSE)
...
findingk_narrowed <- searchK(outs_lda$documents, outs_lda$vocab, K = c(10:20),
 prevalence =~ party + tone, data = outs_lda$meta, verbose=FALSE)
...
docs = Corpus(VectorSource(ads$texts))
...
# calculating lda tuning metrics - 10-30
...
We want to minimize the Arun and CaoJuan metrics, and we want to maximize the Deveaud and Grifiths methods. First, we calculated these metrics for a large K range (2-100) with large steps to observe the general pattern. It seems like the Arn and CaoJuan metrics decrease over time, aand the Arun metric increases over time, but the Deveaud metric has a local peak somewhere from 10-20. We then narrowed in on a K range from 5-25. From the plots, it seems that 10 topics may be the best K when taking all 4 metrics intoo account. While the Arun, CaoJuan, and Griffiths metrics are still moving, they start to slightly plateau, and the Deveaud metric has a local peak. 
...
docs=lexicalize(temp_lda$vocab)
...
result[[3]]=slda.em(documents=docs[[1]],
		K=k,
		vocab=vocab,
		num.e.iterations=10,
		num.m.iterations=4,
...
```{r}
result_2d = stm(documents=outs_lda$documents,vocab=outs_lda$vocab,K=k,
	prevalence=~party+tone,data=outs_lda$meta)
```
...
```{r}
result_2e = stm(documents=outs_lda$documents,vocab=outs_lda$vocab,K=k,
	prevalence=~party+tone+office+policy+as.factor(state),data=outs_lda$meta)
```
