...
names(result[[1]])
# [1] "assignments"     "topics"          "topic_sums"      "document_sums"   NA
# [6] NA                NA                NA                NA                "log.likelihoods"

top.topic.words(result[[1]]$topics,20)
result[[1]]$assignments

result[[1]]$topic_sums
result[[1]]$document_sums
```
...
top.topic.words(result[[2]]$topics,20)
result[[2]]$assignments

result[[2]]$topic_sums
result[[2]]$document_sums

# model estimates that map how topics predict y
result[[2]]$coefs
# e.g., order topic words from 'most D' to 'most R'
rbind(result[[2]]$coefs,top.topic.words(result[[2]]$topics,20))[,order(result[[2]]$coefs)]
```
...
t(labelTopics(model=result[[3]], topics = NULL, n = 20, frexweight = 0.5)[1]$prob)
