...
We will only be using the non-stopwords in the analysis that follows. Our 
...
*What is your classification?*
We report classifications for each method at the bottom of this document for easy comparison. 
...
```{r eval=F}
w.train=fightinW(x=x.train,y=y.train)
most_h=names(sort(abs(w.train),decreasing=T)[1:5])
least_h=names(sort(abs(w.train),decreasing=F)[1:5])

words <- data.frame(most_h,least_h) 
```

*Produce Hamilton-discrimination score* (TODO: what are these?)
```{r eval=F}
#class code sos i 
#go through the scores that come out of the rates 
#rates with log odds and then dirichlet fix 
#take the PjH score then do the log odds part which gives scoring 
#logO where its rate of hamilton/1-rate of ham 
#including laplass 
#last line logOham - logOmad 
...
table(test2,y.test)
```
Again the same classification results whether likelihood PrT or PrU is used, we arbitrarily choose to use test1 to calculate precision, accuracy, and recall.
...
#naivebayes.prediction <- test.pred
#naivebayes_results <- apr_general(as.numeric(test1), y.test)
```
...
```{r}
resultsMat <- rbind(stop_results, nonstop_results, fightingW_results, bayes_results)
