...
The following 5 words are scored the most:
```{r}
print(top)
```

The following 5 words are the least Hamiltonian?
```{r}
print(bottom)
```
...
#class code sos i 
 from dictionW() code
	w=(dirch.1-dirch.0)
	outs[[4]]=dirch.1-dirch.0
	
# can always reproduce a party score with this
w=outs[[4]]
...
p.score=rowSums(z)
p.pred=as.numeric(p.score>mean(p.score))
```
