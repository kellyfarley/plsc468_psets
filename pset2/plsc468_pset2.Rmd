...
With the function testTrim(), we try sample cutoffs between 0 and 14: 0.5, 1, 5, and 10 to try to attain the best model. The resulting testing and predicting results are stored in the global variable cutoffPredictions.
...
At different cutoff values, we note that the model gives the best accuracy with the third cutoff (5). We choose the results from this cutoff of 5 to compile into the comparative matrices at the end of this report.

```{r}
stop.prediction <- as.numeric(strsplit(cutoffPredictions$pred.prediction[3], " ")[[1]])
stop_results <- as.numeric(resultsDf[3, ])
```
...
We note, at different cutoff values, the model gives the exact same classification, resulting in the same accuracy, precision, and recall. It is therefore arbitrary which cutoff we choose. We arbitrarily choose the results from the cutoff of 0.1 to compile into the comparative matrices at the end of this report, noting that the results are the exact same from any other cutoff.
...
*What is your classification?*
Our classificatons are reported in a comparative matrix, below. 
...
We sort the non-stop words by frequency to identify the top five most and least "Hamiltonian" words. Results are stored in the "words" data frame. 
```{r}
w.train=fightinW(x=x.train,y=y.train)
top=names(sort(abs(w.train),decreasing=F)[1:5])
bottom=names(sort(abs(w.train),decreasing=T)[1:5])

words <- data.frame(top,bottom) 
```

*Produce Hamilton-discrimination score* (TO FIX)
...
hamiltonW=function(x,y){
 ...
 # rate.0=sum.0/sum(sum.0) #madison
  ...
  #adding in dirch weights 
  rate.1=dirch.1/sum(dirch.1) #hamilton rate
  rate.0=dirch.0/sum(dirch.0) 
  
    
  #odds of word use
  odds.1=rate.1/(1-rate.1) #odds of word in hamilton
  odds.0=rate.0/(1-rate.0) #odds of word in madison
  sj=log(odds.1)-log(odds.0) #sj
  #sj[which(rate.1==0 | rate.0==0)]=NA  #not sure what this is 
   

  return(sj)
}
hamiltonW(x.train, y.train)
...
Interestingly, naiveBayes() classifies all papers as Madison (0), which is the opposite one would expect based on the prior, which is primarily Hamilton papers.
