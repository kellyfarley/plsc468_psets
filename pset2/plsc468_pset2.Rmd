...
fightinW=function(x,y){
  ...
  dirch.0=(sum.0+a.0)/(sum(sum.0)+sum(a.0))
	cor.test(dirch.1,rate.1)
	cor.test(dirch.0,rate.0)
  
  # from dictionW() code
	w=(dirch.1-dirch.0)
	outs[[4]]=dirch.1-dirch.0
#	n =log(odds.1)-log(odds.0)
#	n[.log= ][which(rate.1==0 | rate.0==0)]=NA
	return(w)
}
```
...
test1[which(test1=='Hamilton')]=0
test1[which(test1=='Madison')]=1
