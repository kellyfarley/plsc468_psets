...
#TO -DO TRIM WEIGHTS + WRITE UP OF CLASSIFCATION
...
Our classifications are stored in the variable pred.prediction, where 1 represents Hamilton authorship and 0 represents Madison authorship. Using only stop predict that there are eight Madison documents (H=0) and 7 Hamilton Ddocuments (H=1)
...
#selecting Top 5 fightin words
w.train=FightinW(x=x.train,y=y.train)
top.non=sort(abs(w.train),decreasing=T)[1:5]
#this seems off, lmk what you guys think 
bottom.non=sort(abs(w.train),decreasing =F)[1:5]
```
