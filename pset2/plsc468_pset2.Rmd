...
ttp <- function(cutoff){
  ...
  w.train[which(abs(w.train)<cutoff)]=0
...
testTrim <- function() {
  ...
  # try different cutoffs
   for(i in 1:length(cutoffValues)){
    thisPrediction <- ttp(cutoffValues[[i]])
...
```{r eval=F}
#this is his set up, i don't think i defined y correclty which is causing some problems 
...
FightinW=function(x,y){
  outs=list()
	sum.1=colSums(x[which(y==1),])
	sum.0=colSums(x[which(y==0),])
	#not totally sure what the "outs" are here 
	outs[[1]]=sum.1-sum.0 
