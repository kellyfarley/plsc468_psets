...
Now, we work with the stop words dtm to make training (50% of known papers), testing (other 50% of known papers), and prediction (all disputed papers) sets.
...
# trimming (TODO: change sd(abs(w.train)) to other values!)
...
Our classifications are stored in the variable pred.prediction, where 1 represents Hamilton authorship and 0 represents Madison authorship, and are compiled into a matrix assigning each disputed paper to an author at the end of this report. Using only stop, we predict that there are 8 Madison documents (H=0) and 7 Hamilton documents (H=1).

Next, we assess the performance of the model by calculating accuracy, precision, and recall. We create the function apr() to do so through this report based on true positives, true negatives, false positives, and false negatives.
...
stop_results contains the vector of accuracy, precision, and recall results that will be used to create the final results matrix found at the end of this report.

## b) Non-stopwords (Kelly, in progress)
...
TO DO: someone please check i think this is wrong 
-essentially he said in OH that we should use the DictionW function and substitute in 
the correct dirichlet instead of averages

```{r}
#a: only non-stop words  
y <- knownpapers$classes
x <- non.dtm

#b: fighting words function 
